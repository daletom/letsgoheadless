...
alt: Contentful to Nuxt Tutorial
---

Welcome to a tutorial using Contentful as a headless CMS with Nuxt.js as a front-end framework.  I will take you through to steps of starting a Nuxt.js app from start to finish.  We will also be creating a new Contentful account as well.  The tutorial will also be utilizing Tailwind CSS as the UI framework, the Nuxt Image module to generate a responsive design with imgix images from Prismic, and deploying with Vercel.<br>

<lite-youtube class="youtube" videoid="oLbsqXT1jcQ" style="background-image: url('https://tom.imgix.net/prismictonuxt_ytthumb.png?auto=format,compress&mark-align=middle,center&w=600&mark=youtubeicon.svg');"></lite-youtube> <br> Here is a link to the live site: https://contentfultonuxt.letsgoheadless.com/.

You can also see the github link here: https://github.com/daletom/contentfultonuxt .  
...
Once it is installed, change directory into the new project.

```bash 
cd "projectname"
```

You will next need to add the `contentful` dependecy to your project.

```bash
npm install --save contentful
```

Now you will need to open up your favorite code editor for the project you created.  Create a folder called `plugins`.  In that folder, create a file called `Contentful.js`.  In that file, enter this info: 
...
You will then need to add Contentful to the `plugins` section of your `nuxt.config.js`.
...
One we make our Contentful account, we will be creating env files with the space and access tokens that we listed in our Contentful.js plugins file. 


## Creating Your Contentful Account

You will now need to create a Contentful account if you do not have one.  Once you have created it, you will need to make two different Content Types.  Our first content type will be a prominent article at the top of our project. I will call this one `teaser`. Our second content type will be a list of posts to go below our prominent article at the top. I will call this one `Blog Post`.

For both content types, you will be adding the same items.  You will need to choose Text, Rich Text, and JSON object. The text field specifically should be a short text section.  If you have never made a Contentful account before, you can get more detailed instructions in my Contentful CMS section here: https://www.letsgoheadless.com/cmss/Contentful.

Once you have created your account, it would be great to create some content as well.  Create one item for your teaser content type and at least one item for your Blog Post content type.  If you would like to copy the exact content I am entering, you can find that here: xxx

Once you have some content, lets connect Nuxt to that Contentful account.  While you are logged in to Contentful, click the gear in the nav bar called Settings.  Then click on `API Keys`.  Click the `Add API Key` button at the top right.  After you name it, you should be able to see the `Space ID` and the `Content Delivery API - access token`.  Copy your Space ID and go back to your code editor.  Create a `.env` file in your root. You will need to add a CONTENTFUL_SPACE section (and paste your Space ID) and a CONTENTFUL_ACCESS_TOKEN section. Go ahead and copy your Content Delivery API - access token and paste it in the second section. Should look something like this:

```
CONTENTFUL_SPACE=123
CONTENTFUL_ACCESS_TOKEN=1234
```
## Adding Code to View Basic Scaffolding

Go back to your code editor and go to the `index.vue` file in your pages folder and remove the `<Tutorial>` component. You can also deleted the `Tutorial.vue` file in the components folder as well.
